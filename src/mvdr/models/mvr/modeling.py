...
class DenseModel(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            scores = torch.matmul(q_reps, p_reps.transpose(0, 1)) # (batch_size, num_passage * k)
            scores = scores.view(effective_bsz, -1)

            k = self.data_args.num_query
            scores = scores.reshape(effective_bsz, -1, k) # (batch_size, num_passage, k)

            max_scores, argmax_indices = scores.max(dim=-1) # (batch_size, num_passage)
            target_ = torch.arange(
                scores.size(0),
                device=scores.device,
                dtype=torch.long
            )
            target = target_ * self.data_args.train_n_passages

            # global contrastive loss
            global_cl = self.cross_entropy(max_scores, target)

            # local contrastive loss
            positive_scores = scores[target_, target] # (batch_size, k)
            positive_argmaxs = argmax_indices[target_, target] # (batch_size,)
            local_cl = self.cross_entropy(positive_scores, positive_argmaxs)

            loss = global_cl + self.train_args.local_cl_ratio * local_cl
    ...
    def encode_passage(self, psg):
        ...
        zs = torch.zeros(k, dtype=torch.long, device=input_ids.device)
        ss = torch.arange(seq_length-k, device=input_ids.device) + 1
        position_ids = torch.cat([zs, ss]).expand((1, -1))

        psg_out = self.lm_p(**psg, position_ids=position_ids, return_dict=True)
        p_hidden = psg_out.last_hidden_state
        if self.pooler is not None:
            p_reps = self.pooler(p=p_hidden)  # D * d
        else:
            p_reps = p_hidden[:, :k] # (num_passages, k, emb_dim)
            p_reps = p_reps.reshape(-1, p_reps.shape[-1]) # (num_passages * k, emb_dim)
...
class DenseModelForInference(DenseModel):
    ...
    def __init__(
        ...
        k = self.data_args.num_query
        seq_length = self.data_args.p_max_len
        zs = torch.zeros(k, dtype=torch.long)
        ss = torch.arange(seq_length-k) + 1
        self.position_ids = torch.cat([zs, ss]).expand((1, -1))
