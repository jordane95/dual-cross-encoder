...
class FaissIndex():
    def __init__(self, faiss_index_path, nprobe: int = 10):
        self.faiss_index = faiss.read_index(faiss_index_path)
        self.faiss_index.nprobe = nprobe

    def search(self, q_reps: np.ndarray, k: int):
        return self.faiss_index.search(q_reps, k)
...
def search_queries(retriever, q_reps, p_lookup, args):
    ...
    logger.info(f'query search latengy: {milliseconds / q_reps.shape[0]}')
...
def main():
    ...
    retriever = FaissIndex(args.index_path)
