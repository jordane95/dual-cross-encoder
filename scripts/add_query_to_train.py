...
def main():
    ...
    parser.add_argument("--dataset_name", type=str, default="Tevatron/msmarco-passage", help="Name of huggingface dataset.")
    ...
    parser.add_argument("--save_path", type=str, help="Path to store the new dataset.", required=True)
    ...
    def add_query(example):
        for pos_psg in example['positive_passages']:
            pos_psg["queries"] = doc2queries[int(pos_psg["docid"])][:args.num_query]
        for neg_psg in example['negative_passages']:
            neg_psg["queries"] = doc2queries[int(neg_psg["docid"])][:args.num_query]
        return example
    
    print("Adding generated queries to the dataset...")
    new_dataset = dataset.map(add_query)
    print(new_dataset[0])

    print("Saving new dataset to {}".format(args.save_path))
    if not os.path.exists(args.save_path):
        os.makedirs(args.save_path)
    new_dataset.to_json(os.path.join(args.save_path, 'train.jsonl'))
